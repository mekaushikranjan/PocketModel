...
export interface SessionMetaData {
  ...
  settingsSource: 'global' | 'custom'; // Use global settings or custom settings
}
...
class ChatSessionStore {
  ...
  newChatSettingsSource: 'global' | 'custom' = 'global';
    ...
    try {
      ...
      for (const session of sessions) {
        ...
        sessionMetadata.push({
          ...
          messages,
          completionSettings,
          settingsSource: 'global', // Default to global settings for existing sessions
        });
      }
  ...
  resetActiveSession() {
    runInAction(() => {
      this.newChatSettingsSource = 'global'; // Reset to default for new chat
  ...
  setActiveSession(sessionId: string) {
    runInAction(() => {
      ...
      this.newChatSettingsSource = 'global'; // Reset for consistency
    });
  }
  ...
  async createNewSession(
    ...
    try {
      ...
      if (metaData.settingsSource !== 'custom') {
        metaData.settingsSource = 'global';
      }
  ...
  async updateSessionSettingsSource(source: 'global' | 'custom') {
    if (this.activeSessionId) {
      const session = this.sessions.find(s => s.id === this.activeSessionId);
      if (session) {
        runInAction(() => {
  ...
  setNewChatSettingsSource(source: 'global' | 'custom') {
    runInAction(() => {
   ...
   * System Defaults → Global User Settings → Session-Specific Settings (only if explicitly modified)
   */
